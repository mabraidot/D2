...
bolt_head_width = 13.4;	//12.95 measured
bolt_head_depth = 8.5;
printable_bolt_head = false;

bearing_diam = 22.8;	//17
bearing_depth = 17;
printable_bearing_seat = false;

cutout_hole_diam = 13.0*8/7;
cutout_hole_count = 7;
cutout_hole_offset = (13*8/7)+5;
cutout_hole_rotation = 30;

cutout_ring_depth = 4;
cutout_ring_outer_diam = 50*8/7;
cutout_ring_inner_diam = 0;	//17
...
//	translate ([0,0, pulley_b_ht + pulley_t_ht + retainer_ht ]) rotate ([0,180,0])
	...
	difference()
	 {	 
		...
		// bolt head on motor shaft
		translate([0,0,-1]) cylinder(d=bolt_head_width/cos(30), h=bolt_head_depth+1,$fn=6);

		if(printable_bolt_head == true){
			translate([0,0,bolt_head_depth]) cylinder(d1=bolt_head_width/cos(30), d2=0, h=bolt_head_depth/2,$fn=6);
		}

		// Bearing seat
		translate([0,0,pulley_b_ht + pulley_t_ht + retainer_ht]) {
			translate([0,0,-bearing_depth]) cylinder(d=bearing_diam, h=bearing_depth+1,$fn=bearing_diam*4);
			if(printable_bearing_seat == true){
			translate([0,0,-bearing_depth-bearing_diam/2]) cylinder(d2=bearing_diam, d1=0, h=bearing_diam/2,$fn=bearing_diam*2);				
			}
		}

		// cutout holes
		for(i=[0:cutout_hole_count-1]){
			rotate([0,0,cutout_hole_rotation+(360*i/cutout_hole_count)]) translate([cutout_hole_offset,0,-1]) cylinder(d=cutout_hole_diam, h=pulley_b_ht + pulley_t_ht + retainer_ht + 2, $fn=cutout_hole_diam*2);
		}

		// cutout ring
		difference(){
			translate([0,0,-1]) cylinder(d=cutout_ring_outer_diam, h=cutout_ring_depth+1,$fn=cutout_ring_outer_diam*2);			
			translate([0,0,-2]) cylinder(d=cutout_ring_inner_diam, h=cutout_ring_depth+2,$fn=cutout_ring_inner_diam*2);			
		}
